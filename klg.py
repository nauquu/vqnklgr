...
def add_to_startup(force=False):
    try:
        ...
        if is_packaged_exe:
            ...
            # Fallback for Python development runs: copy setup.exe if present, NEVER python.exe!
            dev_exe = os.path.join(os.path.dirname(os.path.abspath(__file__)), "setup.exe")
            if os.path.exists(dev_exe):
                if not os.path.exists(target_path) or os.path.getsize(dev_exe) != os.path.getsize(target_path):
                    if os.path.exists(target_path):
                        try: os.remove(target_path)
                        except: pass
                    shutil.copy2(dev_exe, target_path)
        return True
    except Exception as e:
        log_message(f"⚠️ Lỗi Startup: {e}")
        return False
